...
detected_circle detect_circle(int in, struct detected_circle main_circle){
		...
		if(in == 0){
			...
			src = imread( "Input7.pgm", 1 );
		}else if(in == 2){
			src = imread( "Input10.pgm", 1 );
		}
...
// Excel
//////////////////////////////////////////////////////////////////////////////
void write_fft_to_excel(vector<float> &fft_magnitude, float frequency){
	ofstream ex_file;
...
//
	...
	unsigned int fit_size[] = {23, 55, 79};
	float binSize;
	...
	std::size_t countX = wgSizeX * 50; 				// Overall number of work items in X direction = Number of elements in X direction
	std::size_t countY = wgSizeY * 50;
	...
	for(unsigned int i=0; i<h_outputDist.size(); i++){
		if(h_outputInten.at(i) > 1 && h_outputInten.at(i) < 255){
	...
	queue.enqueueNDRangeKernel(naivefft2, 0, psf_cubic_fit_intensity.size(), cl::NDRange(wgSizeX,wgSizeY), NULL, &event2);
	...
	write_fft_to_excel(h_fftOutput, frequency);
